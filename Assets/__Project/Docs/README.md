...
| [Hub Staging (The Junkyard)](hub-staging.md) | The pre-run staging ground (O1): Journey → the Hub scene; a deterministic 1-of-3 starting-part offer drawn from the tasted-forms catalog (bare launch always allowed), an independent entry-homeland pick (window-0 override, seeded climb unchanged), data-authored cauldron-voice commentary, launch as the new-run commit point, and the death return (Hades reform, no game-over screen) |
...
| [Environment Dressing](environment-dressing.md) | The world's decoration layer behind one swappable dressing-kit contract (Track E): whole-kit binding + demo quarantine + bind-time tone treatment (E1), the biome feature-pool placement model — clustered decoratives, sparse whole-cell blockers on the now-live `BlockedCells` seam — with Desert/Forest demo kits (E2), and site/camp dressing consuming `SiteStamp.DressingThemeId` — shared skyline, gate, campfire ring (E3) |
