...
- R24a. Entering feeding mode **drops the stage camera** by `InventoryConfig.FeedingCameraDrop` (over `FeedingFramingDuration`) so the area **below the pot** comes into view; leaving feeding raises it back. Crafting frames the slots above the pot; feeding frames the feeding slots below it.
- R25. In feeding mode, clicking a pot bubble moves its artifact into a **feeding tray below the pot** (3D bubbles, like the crafting slots above); clicking a tray item returns it to the pot. The tray holds up to `InventoryConfig.FeedingSlotCount` artifacts (default 3, designer-configurable). A small **readout canvas to the right of the pot** shows the **cumulative archetype weights** of the current tray selection, the **digestion progress** this stage, and the **dominant archetype(s)** accumulated this stage.
- R26. The **Feed** button (on the readout canvas) digests the tray: each artifact maps via `ArtifactArchetypeMapper.ToProfile` into the Mutation subsystem's per-stage tally (`IMutationTally.Add`) and advances digestion (`IDigestionProgress.AddArtifact`); the digested artifacts are consumed (they already left the inventory when selected). See [Mutation Subsystem](mutation-subsystem.md). Feeding only *fills* the tally/digestion; the **stage-up mutation choice** that consumes the ready signal (offering body-part options and resetting the stage) is owned by the Mutation subsystem, not Inventory.
...
- **`IFeedingSession` / `FeedingSession`** — the feeding tray (R25/R26): constructed with a `capacity` (`InventoryConfig.FeedingSlotCount`); `TrySelect(instanceId)` moves an inventory item into the tray (removing it from the inventory, like crafting staging) and returns false when the tray is full, `TryUnselect` returns it, `Consume()` empties the tray and returns the snapshot **without** re-adding it (the caller digests it), `ReturnAll()` returns everything to the inventory; `Tray`, `OnTrayChanged`. No mutation dependency — turning tray items into archetype contributions is the presenter's job.
...
- **`Resources/Prefabs/InventoryStage.prefab`** — the detached diorama, every object on layer `InventoryFocus` (8): root (`InventoryStageView` + `PotView`), `StageCamera` (perspective FOV 35 at local z −1.6, URP **Overlay** render type, culling mask = `InventoryFocus`, `StageClickRouter` raycasts bubble clicks; disabled until the inventory opens), `StageLight` (directional, lights only the stage layer), `Cauldron` (`CauldronView` builds the lathe mesh at runtime, `M_PotBody`), `LiquidSurface` (`LiquidSurfaceView` builds and animates the sector mesh, `M_PotLiquid`, child `SurfaceBubbles` particle system), `Steam` (looping plume, started/stopped by `PotView.SetPotFocused`), `BubbleContainer`, `CraftingSlotsAnchor` (`Slot1`, `Slot2`, `ResultAnchor`, `PuffSuccess`/`PuffFail` steam bursts) with `CraftingSlotsView`, and a `FeedingArea` carrying `FeedingView` — a `FeedingTrayAnchor` **below** the pot (3D tray bubbles, reusing `Bubble.prefab`) and a `FeedingReadoutCanvas` (a ScreenSpace-Overlay `Canvas`, sortingOrder 40, anchored to the right of the pot) holding the cumulative-archetype label, digestion progress bar (`Image` type Filled), dominant-archetype label, and the **Feed** button. `InventoryStageView` references the overlay `StageCamera` it drops/raises for feeding framing (R24a). **The `FeedingArea` is generated by the editor tool — see §4.**
- **`Resources/Prefabs/UI/InventoryHud.prefab`** — `InventoryHudView` with the open/close buttons plus a **feed-mode toggle** button and an active-state indicator (R24), **also generated by the editor tool**.
...
**Feeding contribution of an artifact**: feeding reuses an artifact's **Archetype Weights** (see above and [Mutation Subsystem](mutation-subsystem.md)) — no separate feeding field. An artifact with no archetype weights still counts as one digested artifact toward the stage threshold but adds nothing to the archetype tally. Tune how many artifacts a stage needs via `Resources/Mutation/MutationConfig.asset` (`DigestionThreshold`); tune the feeding-tray capacity and the feeding camera drop on `Resources/Configs/InventoryConfig.asset` (`Feeding Slot Count`, `Feeding Camera Drop`, `Feeding Framing Duration`).

**Building the feeding UI (editor tool)**: run **Tools → Inventory → Setup Feeding UI** to generate the feeding objects on both prefabs without hand-authoring — on `InventoryStage` it creates the `FeedingArea` (below-pot `FeedingTrayAnchor` + right-side `FeedingReadoutCanvas`) and a wired `FeedingView`; on `InventoryHud` it adds the feed-mode toggle button + indicator wired into `InventoryHudView` (`Scripts/Editor/Inventory/FeedingUISetup.cs`). The tool is idempotent — rerun it after changing the layout and it rebuilds cleanly. Adjust positions/styling on the generated objects afterward to taste.
...
- `FeedingSessionTests` — `TrySelect` removes from inventory and adds to the tray (raising `OnTrayChanged`); unknown id rejected; **select past capacity rejected (item stays in inventory)**; `TryUnselect` returns to inventory; `Consume` clears the tray without returning items; empty-tray `Consume` is empty; `ReturnAll` returns every tray item; null inventory and capacity below 1 throw.
