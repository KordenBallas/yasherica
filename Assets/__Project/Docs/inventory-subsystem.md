...
|---|---|---|
| Domain (pure C#) | Container, crafting state machine, recipe matching, bubble layout / drift / cauldron profile / liquid wave math | `Scripts/Inventory/Core/`: `ArtifactInstance.cs`, `InventoryModel.cs`, `CraftingSession.cs`, `RecipeBook.cs`, `BubbleLayoutCalculator.cs`, `BubbleDriftCalculator.cs`, `CauldronProfileCalculator.cs`, `LiquidWaveCalculator.cs` |
...
| Application | Open/close orchestration, crafting flow | `Scripts/Inventory/Presenter/InventoryPresenter.cs`, `CraftingPresenter.cs` |
| Infrastructure | HUD, inventory stage, cauldron/liquid meshes, bubbles, steam, puffs, stage click routing | `Scripts/Inventory/View/`: `InventoryHudView.cs`, `InventoryStageView.cs`, `PotView.cs`, `BubbleView.cs`, `StageClickRouter.cs`, `CraftingSlotsView.cs`, `CauldronView.cs` + `CauldronMeshBuilder.cs`, `LiquidSurfaceView.cs` + `LiquidSurfaceMeshBuilder.cs` |
...
- `IInventoryModel`, `ICraftingSession` (with `ItemsToCombine`), `BubbleLayoutCalculator`.
...
- Both presenters via `BindInterfacesAndSelfTo` + `NonLazy`.
...
**New artifact**: create an `ArtifactDefinition` via *Create → Inventory → Artifact* in `Resources/Artifacts/Definitions/`; set a unique `Id`, display name, icon sprite, and bubble tint. Duplicate or empty ids fail fast at startup (`ArtifactCatalog`).
